...
export function concernObservationPrompt(concerns: PartnerConcern[], workspaceRoot?: string): string {
  ...
  return [
    ...
    '知识文档明确要求用本地命令核验版本、仓库状态或环境事实时，必须调用 heartbeat_local_command 执行文档给出的只读命令；不要改成 glob、grep 或 read 去猜结果。该工具只接受有限的只读检查形式，不经过 shell；命令被拒绝、失败或超时时，必须如实视为“本地事实未核到”，不得寻找相似文件制造结论。知识文档没有明确要求时不要自行调用本地命令。',
    '在缺少可靠的新证据时，选择与挂念直接相关的工具核验；工具可自由组合，已有结果足够时停止，不做无关或重复搜索。不得读取伙伴记忆、会话归档、日记或 concerns 数据库；除 heartbeat_local_command 接受的受控只读核验外，不得执行其他命令、发布、提交或操作其他外部系统。',
