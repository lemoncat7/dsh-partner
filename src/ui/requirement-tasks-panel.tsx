...
import { TaskContractPanel } from './task-contract-panel.js'
...
export function RequirementTasksPanel({ initialTaskId, openRequest, requirementId, back, detailOnly = false }: { initialTaskId?: string | undefined; openRequest?: object | undefined; requirementId?: string; back?(): void; detailOnly?: boolean } = {}): JSX.Element {
  ...
  const accept = async (task: BoardTaskView, checks?: TaskReviewCheck[]): Promise<void> => {
    setBusy(task.id); setError(undefined)
    try { await api(`/tasks/${task.id}/accept`, { method: 'POST', body: JSON.stringify({ expectedRevision: task.revision, checks }) }); await load() }
    catch (reason) { setError(errorMessage(reason)) } finally { setBusy(undefined) }
  }
  const reject = async (task: BoardTaskView, reason: string, checks?: TaskReviewCheck[]): Promise<void> => {
    if (!reason) return
    setBusy(task.id); setError(undefined)
    try { await api(`/tasks/${task.id}/reject`, { method: 'POST', body: JSON.stringify({ reason, expectedRevision: task.revision, checks }) }); await load() }
  ...
  return <div className="dsh-partner-feature-page is-board">
    ...
    {selectedTask && <WorkspaceDialog eyebrow="TASK DETAIL" title={selectedTask.title} detail={taskDialogSummary(selectedTask, directory)} close={() => setSelectedTaskId(undefined)} width="wide"><TaskDetail
      ...
      accept={checks => { void accept(selectedTask, checks) }} reject={(reason, checks) => { void reject(selectedTask, reason, checks) }} remove={() => { void remove(selectedTask) }}
...
function TaskCard({ task, tasksById, directoryById, dependentCount, execution, open }: {
  ...
  const executionState = task.scheduling ? { label: task.scheduling.message } : execution ? delegationState(execution, blockers.length) : task.autoRun && task.status === 'ready' ? { label: blockers.length ? '等待前置完成' : '等待调度' } : undefined
...
function TaskDetail({ task, tasks, activities, directory, execution, busy, update, delegate, review, accept, reject, remove }: {
  ...
  update(change: Record<string, unknown>): void; delegate(): void; review(): void; accept(checks?: TaskReviewCheck[]): void; reject(reason: string, checks?: TaskReviewCheck[]): void; remove(): void
}): JSX.Element {
  const [rejecting, setRejecting] = useState(false)
  const [checks, setChecks] = useState<TaskReviewCheck[]>([])
  useEffect(() => { setChecks([]) }, [task.id, task.revision])
  const [rejectReason, setRejectReason] = useState('')
  useEffect(() => { if (task.status !== 'review') { setRejecting(false); setRejectReason('') } }, [task.status])
  ...
  return <div className="dsh-partner-task-detail">
      ...
      {rejecting && <form className="dsh-partner-task-reject" onSubmit={event => { event.preventDefault(); const reason = rejectReason.trim(); if (!reason) return; reject(reason, checks) }}><label><span>打回原因</span><textarea autoFocus value={rejectReason} onChange={event => setRejectReason(event.target.value)} rows={3} maxLength={1200} placeholder="说明需要重做或补充的内容" /></label><div><button type="button" onClick={() => { setRejecting(false); setRejectReason('') }}>取消</button><button type="submit" className="is-primary" disabled={busy || !rejectReason.trim()}>确认打回</button></div></form>}
      ...
      <footer className="dsh-partner-task-actions"><small>r{task.revision} · {new Date(task.updatedAt).toLocaleString()}</small><span>{task.status === 'review' ? <>{task.reviewerCompanionId && <button type="button" disabled={busy} onClick={review}>{busy ? '核验中…' : `交给 @${reviewer?.name ?? '伙伴'} 核验`}</button>}<button type="button" disabled={busy} onClick={() => setRejecting(true)}>打回重做</button><button type="button" className="is-primary" disabled={busy || Boolean(task.acceptanceCriteria?.length && (checks.length !== task.acceptanceCriteria.length || checks.some(c => c.verdict !== 'passed' || !c.evidence?.trim())))} onClick={() => accept(checks)}>验收通过</button></> : <button type="button" disabled={busy || !canExecute} title={blockers.length ? '可提前排队，依赖通过验收后自动执行' : !task.assigneeCompanionId ? '请先选择负责人' : undefined} onClick={delegate}>{busy ? '执行中…' : execution?.status === 'queued' ? '已提交，等待执行' : task.status === 'doing' ? '正在执行' : task.status === 'done' ? '已经完成' : `交给 @${assignee?.name ?? '伙伴'}`}</button>}<button type="button" className="is-icon" aria-label={`删除 ${task.title}`} aria-expanded={confirmingDelete} disabled={busy} onClick={() => setConfirmingDelete(true)}><IconTrashOutline16 size={14} /></button></span></footer>
  </div>
}
