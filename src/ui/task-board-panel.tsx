...
function TaskForm({ companions, close, changed }: { companions: PartnerDirectoryEntryView[]; close(): void; changed(): Promise<void> }): JSX.Element {
  ...
  return <form className="dsh-partner-task-form" onSubmit={event => { void submit(event) }}><label><span>任务名称</span><input name="title" maxLength={200} required autoFocus /></label><label><span>负责人</span><select name="assignee" defaultValue=""><option value="">未指派</option>{companions.map(item => <option key={item.id} value={item.id}>@{item.name}</option>)}</select></label><label className="is-wide"><span>任务说明</span><textarea name="description" maxLength={8000} rows={4} /></label><label><span>优先级</span><select name="priority" defaultValue="normal"><option value="low">低</option><option value="normal">普通</option><option value="high">高</option><option value="urgent">紧急</option></select></label><footer><button type="submit"><IconPlusOutline16 size={14} />创建任务</button><button type="button" onClick={close}>取消</button></footer>{error && <p role="alert">{error}</p>}</form>
}
