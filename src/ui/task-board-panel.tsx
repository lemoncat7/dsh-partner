...
import { api, type BoardTaskStatusView, type BoardTaskView, type CompanionView, type PartnerDirectoryEntryView, type TaskBoardView } from '../client-api.js'
...
export function TaskBoardPanel({ companion }: { companion: CompanionView }): JSX.Element {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setBoard(next); setDirectory(collaboration.companions); setError(undefined)
  ...
  const delegate = async (task: BoardTaskView): Promise<void> => {
    if (!task.assigneeCompanionId || task.assigneeCompanionId === companion.id) return
    setBusy(task.id)
    try {
      await api(`/tasks/${task.id}/delegate`, { method: 'POST', body: JSON.stringify({ fromCompanionId: companion.id, to: task.assigneeCompanionId, request: task.description || `完成任务：${task.title}` }) })
      await load()
    } catch (reason) { setError(message(reason)) } finally { setBusy(undefined) }
  }
  ...
  return <div className="dsh-partner-feature-page is-board">
    <header className="dsh-partner-feature-hero"><span><small>SHARED WORKSPACE</small><h2>伙伴任务看板</h2><p>人和伙伴共享同一份任务状态；委派会启动真实临时会话，结果进入待验收，而不是只留下一个 @。</p></span><div><button type="button" onClick={() => { void load() }}><IconRefreshOutline16 size={15} />刷新</button><button type="button" onClick={() => setCreating(true)}><IconPlusOutline16 size={15} />新任务</button></div></header>
    ...
    <div className="dsh-partner-board" aria-label="任务看板">{COLUMNS.map(column => {
      ...
      return <section key={column.id} data-status={column.id}><header><strong>{column.label}</strong><b>{tasks.length}</b></header><div>{tasks.map(task => <article key={task.id}>
        ...
        <footer><small>r{task.revision} · {new Date(task.updatedAt).toLocaleDateString()}</small><span>{task.assigneeCompanionId && task.assigneeCompanionId !== companion.id && <button type="button" disabled={busy === task.id} onClick={() => { void delegate(task) }}>{busy === task.id ? '执行中…' : '委派执行'}</button>}<button type="button" className="is-icon" aria-label={`删除 ${task.title}`} disabled={busy === task.id} onClick={() => { void remove(task.id) }}><IconTrashOutline16 size={14} /></button></span></footer>
