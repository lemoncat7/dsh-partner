...
export function MemoryPanel({companion, snapshot, openSession, startSession, renewSession, onChanged}: {
  ...
  const retries = layers.data?.jobs.filter(job => job.status === 'retrying').length ?? 0
  ...
  return <div className="dsh-partner-memory-page">
    ...
    {!!layers.data?.jobs.length && <details className="dsh-partner-memory-jobs"><summary>{retries ? `${retries} 项等待重试` : `${layers.data.jobs.length} 项正在排队整理`}<small>不影响继续聊天</small></summary><div>{layers.data.jobs.slice(0, 10).map(job => <div key={job.id}><span>{job.status === 'processing' ? '正在整理' : job.status === 'retrying' ? `已重试 ${job.attempts} 次 · 下次 ${new Date(job.nextAt).toLocaleTimeString()}` : '等待整理'}</span>{job.status !== 'processing' && <button type="button" disabled={busy} onClick={() => {void run(async () => {await api(`${base}/memory/jobs/${encodeURIComponent(job.id)}?${new URLSearchParams({scopeId})}`, {method: 'POST'})}, '已安排重试')}}>立即重试</button>}</div>)}</div></details>}
