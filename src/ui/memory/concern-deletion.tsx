...
export function ArchivedConcerns({companionId, onChanged}: {companionId:string; onChanged():Promise<void>}) {
  ...
  const resource=useMemoryResource<{items:ConcernView[];hasMore:boolean}>(`/companions/${encodeURIComponent(companionId)}/concerns/archived?offset=${offset}`,15_000)
  const refresh=async()=>{if(offset>0 && resource.data?.items.length===1)setOffset(offset-50);else await resource.reload();await onChanged()}
  if (!resource.error && (!resource.data || (offset===0 && resource.data.items.length===0))) return null
  return <section className="dsh-partner-concern-archive"><button type="button" className="dsh-partner-concern-archive-toggle" aria-label="已停止关注 · 查看与清理" aria-expanded={open} aria-controls={open?contentId:undefined} onClick={()=>setOpen(!open)}><span><strong>已停止关注</strong><small>不再关注或长期未活跃而自动停止的记录，可按需清理</small></span><IconChevronDownOutline14 size={16}/></button>{open && <div id={contentId} className="dsh-partner-concern-archive-content">
    {resource.error && <WorkspaceNotice>{resource.error}<button type="button" onClick={()=>{void resource.reload()}}>重试</button></WorkspaceNotice>}
    {!resource.data ? !resource.error && <p role="status">正在读取…</p> : <>{resource.data.items.length===0 && <p>此页没有已停止关注。</p>}<div className="dsh-partner-concern-archive-list">{resource.data.items.map(item=><article key={item.id} className="dsh-partner-concern-archive-row"><details className="dsh-partner-concern-archive-details"><summary><IconChevronDownOutline14 size={14}/><strong>{item.subject}</strong></summary><dl><dt>执行说明与记录格式</dt><dd>{item.reason || '未填写'}</dd><dt>检查依据</dt><dd>{item.resources?.map(source=>source.label || source.locator).join('、') || '未关联'}</dd><dt>记录位置</dt><dd>{item.recordTarget?.label || '伙伴内部观察记录（未指定文档）'}</dd></dl><p>已停止后续检查。删除关注不会删除关联文档。</p></details><div className="dsh-partner-concern-archive-action"><ConcernDeleteButton companionId={companionId} item={item} onDeleted={refresh}/></div></article>)}</div>
    {(offset>0 || resource.data.hasMore) && <nav className="dsh-partner-concern-archive-pages" aria-label="已停止关注分页"><span>第 {Math.floor(offset/50)+1} 页</span><div><button type="button" disabled={offset===0 || resource.loading} onClick={()=>setOffset(Math.max(0,offset-50))}>上一页</button><button type="button" disabled={!resource.data.hasMore || resource.loading} onClick={()=>setOffset(offset+50)}>下一页</button></div></nav>}</>}
  </div>}</section>
}
