...
export function SchedulePanel({ companions }: { companions: CompanionView[] }): JSX.Element {
  ...
  return <div className="dsh-partner-feature-page">
    <header className="dsh-partner-feature-hero"><span><small>EPHEMERAL AUTOMATION</small><h2>伙伴定时任务</h2><p>创建计划时指定执行伙伴；每次运行使用独立临时会话，默认完成即销毁。</p></span><div><button type="button" onClick={() => { void load() }}><IconRefreshOutline16 size={15} />刷新</button><button type="button" disabled={companions.length === 0} onClick={() => setCreating(true)}><IconPlusOutline16 size={15} />新计划</button></div></header>
    {creating && <ScheduleForm companions={companions} close={() => setCreating(false)} changed={load} />}
...
function ScheduleForm({ companions, close, changed }: { companions: CompanionView[]; close(): void; changed(): Promise<void> }): JSX.Element {
  ...
  return <form className="dsh-partner-schedule-form" onSubmit={event => { void submit(event) }}><label><span>计划名称</span><input name="title" required maxLength={160} autoFocus /></label><label><span>执行伙伴</span><select name="companionId" required defaultValue={companions[0]?.id ?? ''}><option value="" disabled>选择伙伴</option>{companions.map(companion => <option key={companion.id} value={companion.id}>@{companion.name} · {companion.role}</option>)}</select></label><label className="is-wide"><span>执行内容</span><textarea name="prompt" required maxLength={12000} rows={4} placeholder="明确说明要检查、整理或完成什么" /></label><label><span>频率</span><select value={kind} onChange={event => setKind(event.target.value as 'interval' | 'daily')}><option value="interval">每隔一段时间</option><option value="daily">每天固定时间</option></select></label>{kind === 'interval' ? <label><span>间隔分钟</span><input name="minutes" type="number" min={5} max={43200} defaultValue={60} /></label> : <div className="dsh-partner-schedule-time"><label><span>小时</span><input name="hour" type="number" min={0} max={23} defaultValue={9} /></label><label><span>分钟</span><input name="minute" type="number" min={0} max={59} defaultValue={0} /></label></div>}<label><span>重叠策略</span><select name="overlapPolicy" defaultValue="skip"><option value="skip">跳过重叠</option><option value="queue">排队执行</option></select></label><label><span>超时分钟</span><input name="timeoutMinutes" type="number" min={1} max={120} defaultValue={10} /></label><label className="dsh-partner-check is-wide"><input name="retain" type="checkbox" /><i /><span>执行后保留临时会话</span></label><footer><button type="submit"><IconPlusOutline16 size={14} />创建计划</button><button type="button" onClick={close}>取消</button></footer>{error && <p role="alert">{error}</p>}</form>
}
