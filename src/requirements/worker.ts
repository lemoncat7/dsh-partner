...
interface RequirementEffects {
  summarize(item: BoardRequirement, tasks: BoardTask[], signal: AbortSignal): Promise<string>
...
export class RequirementWorker {
  ...
  private async run(): Promise<void> {
    ...
    for (const item of snapshot.requirements ?? []) {
      ...
      if (!summarize && !(item.status === 'done' && !item.notifiedAt)) continue
...
export function requirementSummaryPrompt(item: BoardRequirement, tasks: BoardTask[]): string {
  const header = [
  ...
  const rows = tasks.map((task, index) => {
    const title = `## ${index + 1}. ${task.title.slice(0, Math.min(80, Math.floor(budget / 3)))}`
    const result = task.resultAbstract || task.resultSummary || '未提供结果正文'
