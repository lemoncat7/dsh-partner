...
import { DEFAULT_PENDANT_SETTINGS, normalizePendantSettings, samePendantSettings, type StrapMaterial, type PendantFps } from './settings.js'
...
export function PendantSettingsPanel(): JSX.Element {
  ...
  const upload = async (event: ChangeEvent<HTMLInputElement>): Promise<void> => {
    ...
    try { const image = await prepareCardImage(selected); if (request === epoch.current) update({ image }) }
    ...
    <div className="dsh-partner-pendant-settings-layout">
      <div className="dsh-partner-pendant-settings-fields">
        <WorkspaceBlock title="主界面挂饰" detail="关闭后停止挂饰渲染与消息轮询，随时可以重新开启。" actions={<button type="button" className="dsh-partner-feature-switch" data-on={draft.enabled} role="switch" aria-label="显示卡片挂饰" aria-checked={draft.enabled} onClick={() => update({ enabled: !draft.enabled })}><i /></button>}><p className="dsh-partner-pendant-setting-hint">消息保留在卡片内侧；正面展示图案，新消息到达时轻摆、转正并闪光。</p>
          <div className="dsh-partner-pendant-fps-row"><span>帧率上限</span><div className="dsh-partner-pendant-fps-options" role="group" aria-label="挂饰帧率">{FRAME_RATES.map(item => <button key={item.fps} type="button" aria-pressed={draft.fps === item.fps} onClick={() => update({ fps: item.fps })}><strong>{item.fps} FPS</strong><small>{item.label}</small></button>)}</div><small>仅影响动画绘制，实际帧率取决于设备；静止时自动休眠。</small></div>
        </WorkspaceBlock>
        ...
        <WorkspaceBlock title="正面图案" detail="支持 PNG、JPEG、WebP，最大 5 MB。图片居中裁切为竖向卡面，只在本机处理。">
          ...
          <p className="dsh-partner-pendant-setting-hint">建议使用 8:11 竖图；保存后刷新仍会保留，消息不会覆盖你的图案。</p>
        </WorkspaceBlock>
