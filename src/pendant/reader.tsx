...
export function PendantReader({ inbox, selected, error, anchor, onSelect, onClose, onVisit, onReadAll }: Props): JSX.Element {
  ...
  return <section ref={panel} id="dsh-partner-pendant-inbox" className="dsh-partner-pendant-inbox" aria-label="伙伴消息" tabIndex={-1} style={placement}>
    <header>
      ...
      <div className="dsh-partner-pendant-reader-heading"><strong>{detail ? detail.companionName : '伙伴消息'}</strong><small>{detail ? detail.kind === 'system' ? '插件系统通知' : '来自伙伴的消息' : '回复与任务动态'}</small></div>
    ...
    {detail ? <><article className="dsh-partner-pendant-detail"><div className="dsh-partner-pendant-reader-meta"><span>{detail.kind === 'system' ? '升级提醒' : detail.kind === 'task' ? '任务动态' : detail.kind === 'schedule' ? '定时任务' : '会话回复'}</span><time dateTime={new Date(detail.createdAt).toISOString()}>{new Date(detail.createdAt).toLocaleString([], { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}</time></div><h3>{detail.title}</h3><NoticeMarkdown source={detail.summary} /></article><footer><button type="button" className="dsh-partner-pendant-all-messages" onClick={() => onSelect()}>全部消息<span>{inbox.items.length}</span></button><button type="button" className="dsh-partner-pendant-source" onClick={() => onVisit(detail)}><IconLinkOutline16 size={15} />{detail.kind === 'system' ? '前往升级迁移' : detail.kind === 'task' ? '查看任务' : detail.kind === 'schedule' ? '查看定时任务' : '进入会话'}</button></footer></>
