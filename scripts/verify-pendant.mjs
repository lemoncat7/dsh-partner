...
try {
  ...
  assert.ok(bufferSize[0] * bufferSize[1] < 120000, '2x high-DPI card still uses a small GPU target')
  const cssSize = await page.locator('canvas').evaluate(el => parseFloat(el.style.width))
  assert.ok(Math.abs(bufferSize[0] / cssSize - 2) < .02, 'retina canvas is not upscaled from 1.5x pixels')
  assert.equal(await page.locator('canvas').evaluate(el => getComputedStyle(el).filter), 'none')
  ...
  const idleGlare = await page.locator('canvas').evaluate(c=>[c.dataset.glare,c.dataset.glareProgress])
  await hit.hover(); await page.waitForTimeout(200)
  assert.deepEqual(await page.locator('canvas').evaluate(c=>[c.dataset.glare,c.dataset.glareProgress]), idleGlare, 'hover does not change angle-based reflection')
  await page.evaluate(() => {
    window.motionGlareSamples = []
    window.glareObserver = new MutationObserver(() => {
      const c = document.querySelector('canvas')
      if (Number(c.dataset.glare) > 0) motionGlareSamples.push(Number(c.dataset.glareProgress))
    })
    glareObserver.observe(document.querySelector('canvas'), {attributes:true, attributeFilter:['data-glare-progress']})
  })
  ...
  const heldGlare = await page.locator('canvas').evaluate(c=>[c.dataset.glare,c.dataset.glareProgress])
  assert.ok(await page.evaluate(() => new Set(motionGlareSamples).size > 2), 'physical movement advances the glare sweep')
  await page.evaluate(() => glareObserver.disconnect())
  await page.waitForTimeout(350)
  assert.equal(await page.evaluate(() => frameCount), heldFrames, 'holding a settled stretch stops the frame loop')
  assert.deepEqual(await page.locator('canvas').evaluate(c=>[c.dataset.glare,c.dataset.glareProgress]), heldGlare, 'held angle retains its fixed reflection without replay')
  await page.evaluate(() => {
  ...
  assert.equal(Number(await page.locator('canvas').getAttribute('data-glare')), 0, 'reduced motion suppresses front glare')
