...
test('recomposes one recovered partner agent exactly once and releases its scoped contributions', async () => {
  ...
  try {
    ...
    await runtime.prepareSession(route.id)
    assert.equal(composerRuns, 0)
    await runtime.prepareAgentTurn(agent, 1, new AbortController().signal)
    ...
    assert.equal(composerDisposals, 0)
    await store.update(state => { state.companions[0].instructions = 'updated instructions' })
    await runtime.prepareAgentTurn(agent, 2, new AbortController().signal)
    assert.equal(composerDisposals, 1)
    assert.deepEqual([...sections].sort(), ['partner-identity', 'partner-tool-routing'])
    assert.equal(maintenanceRuns, 0)
    assert.equal(composerRuns, 2)
    await runtime.prepareSession(route.id)
    await runtime.prepareAgentTurn(agent, 3, new AbortController().signal)
    assert.equal(maintenanceRuns, 0)
    assert.equal(composerRuns, 2)
    await runtime.close()
