...
test('continuation requires explicit ownership and never manufactures a requirement from dependencies', async t => {
  ...
  await requirementTool.execute({ action: 'submit', requirementId: req.id, expectedRevision: current().revision }, exec)
  await assert.rejects(taskTool.execute({ action: 'create', requirementId: req.id, title: '继续视觉', autoRun: false }, exec), /规划状态/)
