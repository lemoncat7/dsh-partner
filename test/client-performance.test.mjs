...
test('workspace exposes the host canvas while dialogs and option popups own opaque surfaces', () => {
  ...
  assert.match(clientCss, /body\[data-ds-dark-theme\][\s\S]*--partner-surface: #1c1c1e;/)
  assert.match(clientCss, /body\[data-ds-dark-theme\][\s\S]*--partner-pane: #2c2c2e;/)
  assert.match(clientCss, /body\[data-ds-dark-theme\][\s\S]*--partner-accent: #69b6ba;/)
  assert.doesNotMatch(clientCss, /body\[data-ds-dark-theme\][\s\S]*--partner-pane: rgba\(8, 10, 12/)
