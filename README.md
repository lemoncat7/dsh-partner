...
- 对话和心跳前按话题召回画像、偏好、任务、事件、关系与短期情绪信号
- 可配置的低打扰心跳：静默时段、检查间隔、每日上限、失败退避和手动检查
- 心跳只有在模型判断存在实际价值时才发送；`NO_ACTION` 不触发微信通知
...
- 伙伴记忆：`<defaultCwd>/partners/<伙伴 ID>/memory/scopes/`，按联系人隔离保存每日完整对话归档、每日回顾和结构化长期记忆；相关记忆会在对话与心跳前按需召回，默认永久保留。
