...
正式版 `1.12.0` 适配 DeepSeek Harness `0.1.2-rc.1`，需要 Node.js `^22.19.0` 或 `>=24.0.0`。DSH 官方 Agent、Session、Settings、Tools 与客户端 UI 依赖均适配 `0.1.2-rc.1` 接口族。
...
关注使用后台轻量模型循环，不创建临时 Agent 或向聊天写入过程消息。只提供关联依据与记录目标，按文档完成代理、登录和查询；工具走来源作用域的 DSH 执行和审批，不自动批准。最多 12 轮工具交互、24 次调用、180 秒；输出截断需按需分页。依据只读，结果仅写指定目标；检查受阻不能当作无变化。工具审计不复制参数正文。任意命令的文件隔离仍依赖 DSH 沙箱，工具名限制不等同文件系统隔离。
