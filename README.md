...
正式版 `1.7.1` 适配 DeepSeek Harness `0.1.2-rc.1`，需要 Node.js `22.19+` 或 `24+`。DSH 官方 Agent、Session、Settings、Tools 与客户端 UI 依赖均锁定到 `0.1.2-rc.1` 接口族。
...
目标伙伴执行中拒绝修改，配置过期须重新读取。配置已保存但会话未能刷新时，工具会返回 `applied: true` 和 `runtime: reopen-required`，应重新打开目标伙伴会话后使用新配置，不要重复提交。此权限不自动开启任何伙伴的记忆、心跳或渠道。
