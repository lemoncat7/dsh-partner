...
单条检查受阻或抛错时，只把该条 `nextCheckAt` 推迟一个已配置心跳间隔，不更新可靠检查基线，不增加伙伴级连续失败计数。更新使用原 `nextCheckAt` 比较，避免迟到执行覆盖新的调度。部分失败记录仍展示失败原因，但成功项的通知当轮发送；全部检查失败也按各条时间重试，不把伙伴锁入六小时指数退避。持久化或通知发送等调度级异常仍保留原来的失败处理与补发机制。执行仍是串行的，单条尚未返回时后续项仍需等待其时限结束。
